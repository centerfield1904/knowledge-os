...
Current production uses the modular path: morning catalog ingest/render through `scripts/run_catalog_ingest.sh`, explicit remote website publish via the `bvaibhav-info` GitHub Action, and afternoon WhatsApp delivery from the existing rendered digest.
...
# Run ingest, trigger website publish, then verify bvaibhav.info
...
cp config/cron.env.example ~/.config/knowledge-os/cron.env
chmod 600 ~/.config/knowledge-os/cron.env
...
- GitHub CLI installed, plus `GH_TOKEN` in `~/.config/knowledge-os/cron.env` for cron-safe website workflow dispatch
- Baileys WhatsApp Web session linked in `~/.config/knowledge-os/baileys-auth`

The website repo checkout and its Node dependencies are only needed if you manually run local website export/build. The remote publish path relies on the pushed digest artifact, cron-safe `GH_TOKEN`, and the remote website GitHub Action.
...
- **`scripts/run_daily_ingest_and_verify_publish.sh`** — manual ops wrapper that runs the morning path and verifies the public website JSON after website publish
