...
CRON_ENV="${KNOS_CRON_ENV:-$HOME/.config/knowledge-os/cron.env}"
...
trigger_website_workflow() {
    ...
    if [ -z "${GH_TOKEN:-}" ] && ! gh auth status --hostname github.com >/dev/null 2>&1; then
        ...
        echo "GitHub CLI is not authenticated and GH_TOKEN is not set." >&2
        echo "Create ${CRON_ENV} with a scoped GH_TOKEN for cron." >&2
        return 1
    fi
